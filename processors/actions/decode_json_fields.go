...
func init() {
	processors.RegisterPlugin("decode_json_fields",
		configChecked(newDecodeJSONFields,
			requireFields("fields", "topic"),
			allowedFields("fields", "topic", "max_depth", "overwrite_keys", "process_array", "target", "when")))
}
...
func newDecodeJSONFields(c common.Config) (processors.Processor, error) {
	...
	f := decodeJSONFields{fields: config.Fields, maxDepth: config.MaxDepth, overwriteKeys: config.OverwriteKeys, processArray: config.ProcessArray, target: config.Target, topic: config.Topic}
	return f, nil
}
