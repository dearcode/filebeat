...
func (client *Client) PublishEvents(data []outputs.Data) ([]outputs.Data, error) {
	...
	for _, d := range data {
		//有kakfkaOffset应该可以直接调用
		if to, err := d.Event.GetValue("type"); err == nil {
			if id, err := d.Event.GetValue(MetaUUID); err == nil {
				var p int32
				var o int64
				fmt.Sscanf(id.(string), "%4x%16x", &p, &o)

				debugf("PublishEvents topic:%v, partition:%v, offset:%v", to, p, o)

				offset.Update(to.(string), p, o)
			}
		}
	}
...
func bulkEncodePublishRequest(
	...
	for _, datum := range data {
		uuid := datum.Event[MetaUUID].(string)
		meta := createEventBulkMeta(index, pipeline, datum, uuid)
		datum.Event.Delete(MetaUUID)
...
func createEventBulkMeta(index outil.Selector, pipelineSel *outil.Selector, data outputs.Data, uuid string) interface{} {
	event := data.Event
